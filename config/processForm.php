...
if( !isset($_FILES['image']) || !isset($_POST['fname']) || !isset($_POST['mname']) || !isset($_POST['lname']) || !isset($_POST['dob']) ||
    ...
    $msg['incomplete_input'] = 'All fields are required';
    ...
    if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
      $msg['invalid-email'] = "Invalid email format";
    ...
    if(preg_match("!image!", $_FILES['image']['type'])){
        if(copy($_FILES['image']['tmp_name'], $image_path)){
            ...
            if(mysqli_query($conn, $sql)){
                $msg['register-success'] = 'Student detailshave been successfully submitted';
                header("location:../records.php");
            }
        }

    }else{
        $msg['inalid_img'] = 'Please upload a valid image type';
    }
}